...
const mockRssService = {
  ...
  getActiveFeedConfigs: vi.fn(),
  getFeedConfig: vi.fn(),
  getFeedConfigs: vi.fn(),
  updateFeedConfig: vi.fn(),
  deleteFeedConfig: vi.fn()
} as unknown as RSSService;

vi.mock('../rss', () => ({
  RSSService: vi.fn().mockImplementation(() => mockRssService)
}));
...
describe('OPMLService', () => {
  ...
  beforeEach(() => {
    mockPool = {} as Pool;
    service = new OPMLService(mockPool, mockRssService);
    vi.clearAllMocks();
  });
  ...
  describe('importOPML', () => {
    it('should successfully import valid OPML content', async () => {
      ...
      mockAddFeed.mockResolvedValueOnce({ id: 1 } as RSSFeedConfig);
      mockAddFeed.mockResolvedValueOnce({ id: 2 } as RSSFeedConfig);
